...
export class WebhookController {
  async sendEvent(
    event: WEBHOOK_CONTROLLER_EVENTS,
    ...
    try {
      const transactionDetails =
        await transactionModel.getWebhookControllerTransactionDetails(
      ...
      if (event === WEBHOOK_CONTROLLER_EVENTS.CHARGE_SUCCESS) {
        ...
        if (
          transactionDetails.transaction_status !== TRANSACTION_STATUS.FAILED
        ) {
          throw new MyError(Errors.TRANSACTION_STATUS_MISMATCH);
        }
        ...
        if (
          transactionDetails.transaction_status !== TRANSACTION_STATUS.ONRAMPED
        ) {
          throw new MyError(Errors.TRANSACTION_STATUS_MISMATCH);
        }
      ...
      await webhookModel.sendEvent(
        webhookData,
        signature,
        transactionDetails.business_webhook,
      );
    } catch (err) {
      logger.error("Webhook Controller Send Event: Error sending event", {
        error: err,
        event,
        transaction_reference,
      });
