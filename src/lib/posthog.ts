...
class Logger {
    ...
    constructor() {
        if (!process.env.POSTHOG_SECRET || !process.env.POSTHOG_URL || !process.env.POSTHOG_DISTINCT_ID || !process.env.NODE_ENV) {
            console.log("Set POSTHOG_SECRET, NODE_ENV, POSTHOG_URL and POSTHOG_SECRET in env variables");
            throw new Error("Set POSTHOG_SECRET, NODE_ENV, POSTHOG_URL and POSTHOG_SECRET in env variables");
        }
