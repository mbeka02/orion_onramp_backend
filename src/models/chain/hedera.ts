...
import {
  API_NODES,
  hederaAccountDetailsSchema,
  hederaTokenBalanceSchema,
  SUPPORTED_CHAINS,
} from "../../types/chain";
import {
  AccountId,
...
export class HederaChainModel {
  ...
  ): Promise<{ public: PublicKey; private: PrivateKey }[]> {
    try {
      if (tokenType === TOKEN_TYPE.KESy_TESTNET) {
        if (
          !process.env.KESy_TESTNET_PRIVATE_KEY ||
          !process.env.KESy_TESTNET_KEY_ACCOUNT_ID
        ) {
          throw new Error(
            "Invalid env, setup KESy_TESTNET_PRIVATE_KEY and KESy_TESTNET_KEY_ACCOUNT_ID in env",
          );
        }

        const key1 = PrivateKey.fromStringECDSA(
          process.env.KESy_TESTNET_PRIVATE_KEY,
        );
        const privateKey2 = await infisical.getSecret(
          InfisicalKeys.KESy_TESTNET_PRIVATE_KEY,
          "staging",
        );
        const key2 = PrivateKey.fromStringECDSA(privateKey2);

        return [
          { private: key1, public: key1.publicKey },
          { private: key2, public: key2.publicKey },
        ];
      } else {
        throw new Error("Unsupported token type");
      }
    } catch (err) {
      logger.error("Hedera Chain Model: Could not get multi signature key", {
        error: err,
      });
  ...
  _getClient(network: "test" | "main"): Client {
    try {
      if (
        !process.env.HEDERA_BACKEND_ACCOUNT_ID ||
        !process.env.HEDERA_BACKEND_PRIVATE_KEY
      ) {
        throw new Error(
          "Invalid env setup, set HEDERA_BACKEND_ACCOUNT_ID and HEDERA_BACKEND_PRIVATE_KEY in env",
        );
      }
  ...
  async _checkIfAssociated(
    ...
    try {
      ...
      logger.error(
        "Hedera Contract: Error checking if account has associated",
        { error: err, token, accountid },
      );
      throw new Error("Error checking if account has associated");
    }
  }

  async transferTokenFromTreasuryToAccount(
    tokenType: TOKEN_TYPE,
    ...
    try {
      if (tokenType === TOKEN_TYPE.KESy_TESTNET) {
        const hasReceiverAssociated = await this._checkIfAssociated(
          tokenType,
          token,
          receiverAccount,
        );
        ...
        const signedTransaction = transaction
          .addSignature(keys[0].public, signature1)
          .addSignature(keys[1].public, signature2);
    ...
    try {
      ...
      if (chain === SUPPORTED_CHAINS.HEDERA_MAINNET) {
        const res = await fetch(
          `${API_NODES.HEDERA_MAINNET}/api/v1/accounts/${accountAddress}/tokens?token.id=${tokenID}`,
          {
        ...
        const res = await fetch(
          `${API_NODES.HEDERA_TESTNET}/api/v1/accounts/${accountAddress}/tokens?token.id=${tokenID}`,
          {
            signal: controller.signal,
          },
        ).finally(() => clearTimeout(timeoutID));
      ...
      if (parsed.error) {
        throw new Error("Could not parse data from hedera", {
      ...
      logger.error("Error getting token balance on Hedera", {
        error: err,
        chain,
        account,
        token,
