...
export class EnvironmentModel {
  async doesBusinessAlreadyHaveEnvironment(
    businessID: string,
    ...
    try {
      ...
      logger.error(
        "Environment Model Error: Error checking if business already has environment",
        { error: err, businessID, environmentType },
      );
      throw new Error("Error checking if business already has environment");
    }
  }

  async storeEnvironment(environment: {
    ...
    try {
      ...
      logger.error("Environment Model Error: Error storing environment", {
        error: err,
        environment,
      });
  ...
  async getLatestValidBusinessEnvironmentKeys(
    business_id: string,
    environment_type: ENVIRONMENT_TYPES,
  ): Promise<{ public_key: string; encrypted_private_key: string } | null> {
    try {
      const environmentKeys = await db
        .select({
        ...
        .innerJoin(
          environmentsTable,
          eq(environmentsTable.id, environmentKeysTable.environmentID),
        )
        .where(
          and(
      ...
      logger.error("Error getting business environment keys", {
        error: err,
        business_id,
        environment_type,
      });
  ...
  async getBusinessEnvironments(business_id: string): Promise<
    Array<{
      id: string;
      type: ENVIRONMENT_TYPES;
      public_key: string;
  ...
  > {
    try {
      ...
      logger.error("Error getting business environments", {
        error: err,
        business_id,
      });
  ...
  async doesPrivateKeyExist(
    ...
    try {
      ...
      const exists = await db
        ...
        .where(
          and(
            ...
            or(
              isNull(environmentKeysTable.expiresAt),
              gt(environmentKeysTable.expiresAt, now),
            ),
          ),
        )
        .limit(1);
      ...
      logger.error("Environment Model: Error checking if private key exists", {
        error: err,
      });
