...
export class LiquidityManagerModel {
  async getCachedTreasuryTokenBalance(token_type: TOKEN_TYPE): Promise<number> {
    try {
      ...
      if (tokenBalance.length < 1) {
        ...
        return divideBigIntWithDecimals(
          tokenBalance[0].balance,
          BigInt(Math.pow(10, tokenBalance[0].decimals)),
        );
      }
    } catch (err) {
      logger.error("Liquidity Model: Error getting cached treasury balance", {
        error: err,
        token_type,
      });
  ...
  async getTreasuryBalanceFromOnchain(
    token_type: TOKEN_TYPE,
    hederaChainModel: HederaChainModel,
    ...
    try {
      ...
      if (tokenDetails.length < 1) {
        ...
        if (
          tokenDetail.chain === SUPPORTED_CHAINS.HEDERA_MAINNET ||
          tokenDetail.chain === SUPPORTED_CHAINS.HEDERA_TESTNET
        ) {
          const { balance, decimals } = await hederaChainModel.getTokenBalance(
            tokenDetail.chain,
            tokenDetail.treasuryAccount,
            tokenDetail.address,
          );
      ...
      logger.error("Liquidity Model: Error getting token balance on chain", {
        error: err,
        token_type,
      });
  ...
  async deductCachedTreasuryBalance(token: TOKEN_TYPE, amount: number) {
    try {
      ...
      logger.error("Liquidity Model: Error deducting cached treasury balance", {
        error: err,
        token,
        amount,
      });
  ...
  async getTransactionDetailsForBusinessTransfer(
    ...
    try {
      ...
      logger.error(
        "Liquidity Manager Model: Could not get details needed for business transfer",
        { error: err, tokenType },
      );
      ...
      throw new Error(
        "Could not get transaction details needed for business transfer",
      );
    }
  }
  ...
  async sendTokensToAccount(details: BusinessTransactionDetails) {
    try {
      ...
      logger.error("Liquidity Manager Model: Error sending tokens to account", {
        error: err,
        details,
      });
      if (err instanceof MyError) {
  ...
  async undoTreasuryCachedBalanceDeduct(token: TOKEN_TYPE, amount: number) {
    try {
      ...
      logger.error(
        "Liquidity Manager Model: Error undoing treasury cache balance deduct",
        { error: err, token, amount },
      );
  ...
  async markTransactionAsOnramped(reference: string) {
    try {
      ...
      logger.error(
        "Liquidity Manager Model: Error marking transaction as onramped",
        { error: err, reference },
      );
