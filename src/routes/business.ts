...
router.get("/user", authenticationMiddleware, async (req, res) => {
  try {
    ...
    const businesses = await businessController.getAllUserBusinesses(
      userId,
      businessModel,
    );
...
router.post("/create", authenticationMiddleware, async (req, res) => {
  try {
    ...
    if (!parsed.success) {
      ...
      logger.error("Business Route: Invalid create data", {
        data: req.body,
        error,
      });
    ...
    const result = await businessController.createDraft(
      parsed.data,
      userId,
      businessModel,
    );
...
router.put("/:id", authenticationMiddleware, async (req, res) => {
  try {
    ...
    if (!parsed.success) {
      const error = parsed.error.issues[0].message;
      logger.error("Business Route: Invalid update data", {
        data: req.body,
        error,
      });
      res.status(400).json({ message: error });
    ...
    const isAllowed = await businessModel.isUserOwnerOrAdmin(
      businessId,
      userId,
    );
    if (!isAllowed)
...
router.put("/submit/:id", authenticationMiddleware, async (req, res) => {
  try {
    ...
    if (!parsed.success) {
      const error = parsed.error.issues[0].message;
      logger.error("Business Route: Invalid submit data", {
        data: req.body,
        error,
      });
      res.status(400).json({ message: error });
    ...
    await businessController.submitForApproval(
      businessId,
      userId,
      businessModel,
    );
...
router.post("/:id/invite", authenticationMiddleware, async (req, res) => {
  try {
    ...
    if (!parsed.success) {
      const error = parsed.error.issues[0].message;
      logger.error("Business Route: Invalid invite data", {
        data: req.body,
        error,
      });
      res.status(400).json({ message: error });
    ...
    const businessId = req.params.id;
    const result = await businessController.inviteUser(
      businessId,
      userId,
      parsed.data,
      businessModel,
...
router.get("/industries", authenticationMiddleware, async (req, res) => {
  try {
    const industries =
      await businessController.getIndustriesAndCategories(businessModel);
