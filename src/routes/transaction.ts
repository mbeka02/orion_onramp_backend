 ...
 * Get transaction details (alternative endpoint)
 */
router.get(
  "/:reference",
  validateParams(verifyTransactionParamsSchema),
  async (req: Request, res: Response) => {
    try {
      ...
      const transaction =
        await transactionModel.getTransactionByReference(reference);
      ...
      logger.error("Error fetching transaction in router", {
        error: err,
        reference: req.params.reference,
      });
