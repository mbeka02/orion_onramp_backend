  ...
  try {
    ...
    for (const token of tokens) {
      const { balance, decimals } =
        await liquidityModel.getTreasuryBalanceFromOnchain(
          token,
          hederaChainModel,
        );
...
export function startCachedTreasuryBalanceUpdate(
  ...
  logger.info(
    `Starting cached treasury balance updates with schedule: ${cronSchedule}`,
  );
  ...
  cronJob = cron.schedule(cronSchedule, async () => {
    logger.info(
      `[${new Date()}] running scheduled cached treasury balance update...`,
    );
    try {
