...
describe("Treasury Business SDK Onramp Tests", () => {
    ...
    beforeAll(async () => {
        treasuryModelMock.doesTransactionExist = jest.fn().mockImplementation((transaction_id: string) => {
            return new Promise((res, rej) => {
                if (transaction_id === existing_transaction_reference || transaction_id === onramped_transaction_reference || transaction_id === not_complete_transaction_reference || transaction_id === too_much_transaction_reference || transaction_id === enough_transaction_reference || transaction_id === failing_transaction_reference) {
                    res(true);
                } else {
                    res(false);
                }
        ...
        treasuryModelMock.isFiatPaymentCompleted = jest.fn().mockImplementation((transaction_id: string) => {
            return new Promise((res, rej) => {
                if (transaction_id === existing_transaction_reference || transaction_id === onramped_transaction_reference || transaction_id === too_much_transaction_reference || transaction_id === enough_transaction_reference || transaction_id === failing_transaction_reference) {
                    res(true);
                } else {
                    res(false);
                }
        ...
        transactionModelMock.getTransactionByReference = jest.fn().mockImplementation((id) => {
            return new Promise((res, rej) => {
        ...
        liquidityManagerControllerMock.markTransactionOnramped = jest.fn().mockImplementation((reference) => {
            return new Promise((res, rej) => {
    ...
    it("should fail if transaction reference doesn't exist", async () => {
        try {
            await treasuryController.businessOnramp(non_existing_transaction_reference, treasuryModelMock, liquidityManagerControllerMock, transactionModelMock, liquidityModelMock, emailServiceMock);
    ...
    it("should fail if transaction exists but has already been onramped", async () => {
        try {
            await treasuryController.businessOnramp(onramped_transaction_reference, treasuryModelMock, liquidityManagerControllerMock, transactionModelMock, liquidityModelMock, emailServiceMock);
            expect(false).toBe(true);
        } catch (err) {
            if (err instanceof MyError) {
    ...
    it("should fail if transaction exists but the fiat payment was not successful", async () => {
        try {
            await treasuryController.businessOnramp(not_complete_transaction_reference, treasuryModelMock, liquidityManagerControllerMock, transactionModelMock, liquidityModelMock, emailServiceMock);
            expect(false).toBe(true);
        } catch (err) {
            if (err instanceof MyError) {
    ...
    it("should error out and contact liquidity manager if the amount is more than in treasury", async () => {
        try {
            await treasuryController.businessOnramp(too_much_transaction_reference, treasuryModelMock, liquidityManagerControllerMock, transactionModelMock, liquidityModelMock, emailServiceMock);
    ...
    it("should send tokens to the DApp", async () => {
        try {
            await treasuryController.businessOnramp(enough_transaction_reference, treasuryModelMock, liquidityManagerControllerMock, transactionModelMock, liquidityModelMock, emailServiceMock);
            ...
            expect(liquidityManagerControllerMock.markTransactionOnramped).toHaveBeenCalledWith(enough_transaction_reference, liquidityModelMock);
        } catch (err) {
            console.error("Unexpected error", err);
            expect(false).toBe(true);
        }
    ...
    it("should undo treasury cache deduct if something failed but had enough balance", async () => {
        try {
            await treasuryController.businessOnramp(failing_transaction_reference, treasuryModelMock, liquidityManagerControllerMock, transactionModelMock, liquidityModelMock, emailServiceMock);
            expect(false).toBe(true);
        } catch(err) {
